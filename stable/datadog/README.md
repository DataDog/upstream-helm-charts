...
| `clusterAgent.admissionController.enabled`                   | If true, enables cluster-wide webhooks are enabled                                                                                                        | `false`                                         |
| `clusterAgent.admissionController.namespaceSelector`         | Decides whether to run the webhook on an object based on whether the namespace for that object                                                            | `nil` (matches all namespaces)                  |
| `clusterAgent.admissionController.objectSelector`            | Decides whether to run the webhook based on if the object has matching labels                                                                             | `nil` (matches all objects)                     |
